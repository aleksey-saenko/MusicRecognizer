...
class EnqueuedRecognitionWorkManagerImpl @Inject constructor(
    ...
    override fun getWorkInfoFlowById(enqueuedId: Int): Flow<EnqueuedRecognitionStatusDo> {
...
private fun WorkInfo?.toEnqueuedStatus(): EnqueuedRecognitionStatusDo {
    return this?.let { workInfo ->
        when (workInfo.state) {
            WorkInfo.State.ENQUEUED -> EnqueuedRecognitionStatusDo.Enqueued
            WorkInfo.State.RUNNING -> EnqueuedRecognitionStatusDo.Running
            WorkInfo.State.CANCELLED,
            WorkInfo.State.SUCCEEDED,
            WorkInfo.State.FAILED -> EnqueuedRecognitionStatusDo.Inactive

            WorkInfo.State.BLOCKED -> throw IllegalStateException(
                "EnqueuedRecognitionWorker implemented without a chain of workers"
            )
        }
    } ?: EnqueuedRecognitionStatusDo.Inactive
}